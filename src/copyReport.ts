...
// Wording agreed with Александр 2026-09-25 before shipping; percentage
// bands and the tie-in to the fix-needed emoji added 2026-09-25 after he
// compared a real GPT vs Claude run and found their percentages weren't
// calibrated the same way (e.g. one language's stylistic ordinal-number
// question scored 45% by GPT, 15% by Claude, despite both effectively
// agreeing it was a style call, not a real error) — giving both models
// the same fixed bands narrows that gap without forcing identical verdicts
// on genuinely subjective style questions, which is fine.
...
- «Вероятность ошибки (%)» — оценивай по шкале:
  ...
  • 60–89% — ошибка вероятна, либо это неконсистентность в переводе: один и тот же термин в разных местах переведён по-разному без причины (например, сперва «футбольное поле», потом «футбольная арена»; сперва «балл», потом «очко»). Оба варианта по отдельности могут быть правильными, но вместе — непоследовательность, которую стоит исправить.
...
- «Нужна ли правка» определяется процентом: 70% и выше → ✅; от 30% до 69% → ⚠️; ниже 30% → ❌. Если своим суждением считаешь нужным отступить от этого соответствия — можно, но кратко поясни почему рядом с находкой.
...
- Остальные находки (опечатки, непереведённые термины, неконсистентность терминологии, полнота перевода и т.п.) — оценивай по существу, своим суждением, как переводчик-эксперт, ориентируясь на шкалу выше.
